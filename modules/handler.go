...
func GetAllBarang(rw http.ResponseWriter, rq *http.Request) {
	db, err := sql.Open("mysql",
		"root:@tcp(167.205.67.248:3306)/inventarislfm")
...
func GetBarangBidang(rw http.ResponseWriter, rq *http.Request, bidang string) {
	...
	rows, err := db.Query("SELECT barang.Kode_Barang, barang.Nama_Barang, barang.Kondisi, barang.Lokasi FROM inventarisbidang INNER JOIN barang ON inventarisbidang.BarangKode = barang.Kode_Barang WHERE inventarisbidang.BidangID LIKE (select ID_Bidang from bidang where Nama_Bidang like?)","%"+bidang+"%")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
...
func GetKondisiByKodeBarang(rw http.ResponseWriter, rq *http.Request, kodebarang string) {
	...
	rows, err := db.Query("select Kode_Barang, Nama_Barang, Kondisi, Lokasi from barang where Kode_Barang like?","%"+kodebarang+"%")
	...
	//scan data from table
	for rows.Next() {
		err := rows.Scan(&barang.Kode_Barang, &barang.Nama_Barang, &barang.Kondisi)
...
func GetLokasiBarang(rw http.ResponseWriter, rq *http.Request, lokasi string) {
	db, err := sql.Open("mysql",
		"root:@tcp(167.205.67.248)/inventarislfm")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func UpdateKondisi(rw http.ResponseWriter, rq *http.Request, Kode_Barang string)  {
	var brg barang
	kb := Kode_Barang
	brgDecoder := json.NewDecoder(rq.Body)
	err := brgDecoder.Decode(&brg)
...
//Fungsi untuk mengupdate lokasi dari barang inventaris
func UpdateLokasi(rw http.ResponseWriter, rq *http.Request, Kode_Barang string) {
	var brg barang
	kb := Kode_Barang
	brgDecoder := json.NewDecoder(rq.Body)
	err := brgDecoder.Decode(&brg)
	...
	st, err := db.Prepare("UPDATE barang SET Lokasi = ? where Lokasi like ?")
	if err != nil{
		log.Fatal(err)
	}
